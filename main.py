...
class H(BaseHTTPRequestHandler):
 def do_GET(self):
  ...
  e=detect(q) if q else "neutre"
  emoji,txt=EMO[e]
  self.send_response(200)
  self.send_header("Content-type","text/html; charset=utf-8")
  self.end_headers()
  h=f"""<html><head><meta charset=utf-8><meta name=viewport content=device-width><style>body{{background:#f0fdf4;padding:10px;font-family:sans-serif}}.card{{background:#fff;padding:14px;border-radius:12px;margin:10px 0}}</style></head><body><h1>NOUNKOUN V11 COEUR {emoji}</h1><div class=card style='font-size:40px;text-align:center'>{emoji} {e.upper()}</div><div class=card>{txt}</div><form><input name=q value='{q}' style='padding:12px;width:70%'><button>OK</button></form><div class=card><b>Nonzo:</b> {txt}</div></body></html>"""
  self.wfile.write(h.encode())

port=int(os.environ.get("PORT","8000"))
print(f"V11 COEUR sur {port}")
